# SEPARATE BIKE_MODELS AND DETECT OUTLIERS ----

# separate_bikes_models(): A tidy function to separate the model column into engineered features
  
# detect_ouliers() : A vectorized function that detects outliers using TRUE/FALSE output
...
function(x) {
  ...
  output_tbl <- data_tbl %>% 
    mutate(outlier = case_when(
      data < limits$limit_lo ~ TRUE,
      data > limits$limit_hi ~ TRUE,
      TRUE ~ FALSE
    ))
